...
uniform sampler2DRect map, map2, interaction, masque;
uniform float K, pixelisation, taille_interaction_sur_taille_global, alpha, aff_interaction, aff_color2;
...
void main (void)
{
 vec2 coord_downsample = texcoord1;

 vec4 zone_interaction = texture2DRect(interaction, coord_downsample.xy* taille_interaction_sur_taille_global); 

 if (pixelisation > 0.)
 {
  coord_downsample = pixelisation * floor(coord_downsample/ pixelisation);
 }

 vec4 mapcoord = texture2DRect_cubic(MyTex, coord_downsample);

 vec4 color = texture2DRect_lin(map, mapcoord.xy*2047. ); 
 vec4 color_interaction = texture2DRect(map2, mapcoord.xy*255.); 


 //gl_FragColor = mix(color, color_interaction, 1.);
 vec4 tmp = mix(color, color_interaction, min(1.,aff_color2 + aff_interaction * zone_interaction.r));

 tmp.a *= alpha;

 vec4 color_masque = texture2DRect(masque, texcoord1.xy * taille_masque_sur_taille_global );

 tmp.a*= color_masque.r;
